...
class MakeDataFrameCedi:
    ...
    def frame_storage_cedi(self):
        ...
        values = DashboardCedi.objects.filter(estibas__gt=0)\
    ...
    def frame_total_status(self):
        ...
        values = DashboardCedi.objects.values('nom_bodega')\
            .annotate(Sum('estiba_transito'))\
            .annotate(Sum('estiba_no_apta'))
        df2 = pd.DataFrame(list(values))
        print(df2)
